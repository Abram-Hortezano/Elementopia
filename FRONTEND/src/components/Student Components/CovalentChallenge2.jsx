...
export default function CovalentChallenge2({ onComplete }) {
    ...
    return (
        <DndContext onDragStart={handleDragStart} onDragEnd={handleDragEnd}>
            ...
            <DragOverlay>{activeId && supplyShelf[activeId] ? (<DraggableAtom id="overlay" type={activeAtomType} />) : null}</DragOverlay>
        </DndContext>
    );
}
...
function DraggableAtom({ id, type, isHidden = false }) {
    ...
    const label = type.charAt(0).toUpperCase() + (type === 'chlorine' ? 'l' : '') + (type === 'sodium' ? 'a' : '');
