...
export default function MapTree() {
  ...
  // Auto-start first lesson if not completed
  useEffect(() => {
    const firstLesson = nodes.find((n) => n.id === 1);
    if (!completedNodes.has(1)) {
  ...
  // Handle node click (✅ allow retake)
  ...
  const handleLessonComplete = () => {
    if (activeLesson) {
      setCompletedNodes((prev) => {
        ...
        console.log("✅ Completed:", activeLesson.id);
        return updated;
      });
    }
      ...
      {!activeLesson && (
        <div className="Node-Container">
          {nodes.map((node) => {
            ...
            const isUnlocked = node.id === 1 || prerequisites.every((p) => completedNodes.has(p));
