...
const generateInitialState = () => {
  ...
  atomsConfig.forEach((atom) => {
    initialState[atom.id] = {
      type: atom.type,
      location: atom.location,
      initialElectrons: atom.valenceElectrons,
    };
    for (let i = 0; i < atom.valenceElectrons; i++) {
      const electronId = `e${electronCount++}`;
      initialState[electronId] = {
        type: "electron",
        location: atom.id,
        draggable: true,
      };
...
export default function IonicBonding({ onComplete }) {
  ...
  const prompts = [
    {
      ...
      description:
        "Here are Sodium (Na) and Chlorine (Cl). Experiment by dragging electrons between them. Click 'Next Step' when you're ready.",
    },
    {
      title: "Step 2: The Transfer",
      description:
        "An ionic bond forms when Sodium gives its one outer electron to Chlorine. Place the electron on Chlorine and click 'Next Step'.",
    },
    {
      title: "Step 3: Ions Form",
      description:
        "Perfect! By losing an electron, Sodium becomes a positive ion (Na⁺), and Chlorine becomes a negative ion (Cl⁻). Opposites attract!",
    },
    {
      title: "Tutorial Complete!",
      description:
        "They snap together to form Sodium Chloride (NaCl), or table salt! You've made an ionic bond. Get ready for the challenge.",
    },
    {
      title: "Challenge: Build MgO",
      description:
        "Form an ionic bond between Magnesium (Mg) and Oxygen (O). Drag electrons from Magnesium to Oxygen until Oxygen has 8 outer electrons.",
    },
  ];
  ...
  const advanceStep = () => {
    const sodiumElectrons = Object.values(items).filter(
      (item) => item.type === "electron" && item.location === "na"
    ).length;
  ...
  function checkChallenge() {
    const electronsOnOxygen = Object.values(items).filter(
      (item) => item.type === "electron" && item.location === "o"
    ).length;
    const electronsOnMagnesium = Object.values(items).filter(
      (item) => item.type === "electron" && item.location === "mg"
    ).length;

    if (
      electronsOnOxygen ===
        items.o.initialElectrons + items.mg.initialElectrons &&
      electronsOnMagnesium === 0
    ) {
  ...
  const renderAtomsIn = (location) => {
    return Object.entries(items)
      .filter(
        ([, item]) => item.location === location && item.type !== "electron"
      )
      .map(([id, item]) => {
        const isFinal =
          (promptStep >= 2 &&
            (item.type === "sodium" || item.type === "chlorine")) ||
          (challengeStatus === "correct" &&
            (item.type === "magnesium" || item.type === "oxygen"));
        return (
          <DropZone
            key={id}
            id={id}
            className={`atom ${item.type} ${isFinal ? "final-state" : ""}`}
          >
  ...
  const renderElectronsOn = (atomId) => {
    const electronsOnThisAtom = Object.entries(items).filter(
    ...
    <DndContext onDragStart={handleDragStart} onDragEnd={handleDragEnd}>
      <div className="lesson-modal ionic-bonding">
        ...
        <div className="controls-area">
          {promptStep < 3 && (
            <button
              className={`next-btn ${challengeStatus}`}
              onClick={advanceStep}
            >
              Next Step
            </button>
          ...
          {promptStep === 3 && (
            <button onClick={advanceStep} className="begin-challenge-btn">
              Begin Challenge
            </button>
          )}
          {isChallenge && challengeStatus !== "correct" && (
            <button
              onClick={checkChallenge}
              className={`check-btn ${challengeStatus}`}
            >
              Check My Bond
            </button>
          )}
  ...
  return (
    <div className="intro-screen">
      ...
      <p className="intro-text">
        Some atoms are givers, and some are takers. In this lesson, you'll see
        how the transfer of electrons creates powerful attractions called ionic
        bonds, forming new compounds.
      </p>
